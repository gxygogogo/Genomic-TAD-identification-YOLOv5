...
def regionBox_generate(WT_sig_mat,chrname,min_num,min_val,max_val):
    ...
    WT1 = torch.from_numpy(WT1)
    #WT1 = WT1.half() if half else WT1.float()  # uint8 to fp16/32
    WT1 = WT1.float()  # uint8 to fp16/32
    #WT1 = torch.tensor(WT1)
    WT1 /= 255.0  # 0 - 255 to 0.0 - 1.0
    WT1 = WT1.unsqueeze(0)
    pred = model(WT1, augment=False, visualize=False)[0]
...
## 并行运行
def detect_per_chromosome(WT_signal_all, chrname):
    dense_df_regionBox_all = pd.DataFrame()
    dense_df_regionBox_pre = pd.DataFrame()
    ...
    print('{} have total regions: {}'.format(chrname, len(filtered_regionID)))
    ...
    for R_ind,region_id in enumerate(filtered_regionID):
        ...
        if rows!=cols:
            continue
        if len(rows_WT):
            dense_df_regionBox = regionBox_generate(WT_signal_mat_i,chrname,min_num_i,min_val,max_val)
    ...
    dense_df_regionBox_all = pd.concat([dense_df_regionBox_all, dense_df_regionBox_pre], ignore_index=True)
    return dense_df_regionBox_all

def run(WT_signal_all, chrnames):
    # 使用Parallel并行处理每条染色体
    detect_dfs = Parallel(n_jobs=-1)(delayed(detect_per_chromosome)(WT_signal_all, chr) for chr in chrnames)
    # 合并所有读取的数据
    detect_df = pd.concat(detect_dfs, ignore_index=True)
    return detect_df
