...
class ArgumentParser:
    ...
    def _add_arguments(self):
        ...
        # image size: 480, 对鉴定效果有影响
        self.parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=480, help='train, val image size (pixels)')
        ...
        self.parser.add_argument('--noautoanchor', action='store_true', help='disable autoanchor check')
        self.parser.add_argument('--evolve', type=int, nargs='?', const=300, help='evolve hyperparameters for x generations')
        self.parser.add_argument('--bucket', type=str, default='', help='gsutil bucket')
        self.parser.add_argument('--cache', type=str, nargs='?', const='ram', help='--cache images in "ram" (default) or "disk"')
        self.parser.add_argument('--image-weights', action='store_true', help='use weighted image selection for training')
        # 提交到GPU
        self.parser.add_argument('--device', default='1', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
        ...
        self.parser.add_argument('--linear-lr', action='store_true', help='linear LR')
        # label-smoothing: 0.5, 对鉴定效果有影响
        self.parser.add_argument('--label-smoothing', type=float, default=0.5, help='Label smoothing epsilon')
        self.parser.add_argument('--patience', type=int, default=100, help='EarlyStopping patience (epochs without improvement)')
        self.parser.add_argument('--freeze', type=int, default=0, help='Number of layers to freeze. backbone=10, all=24')
        self.parser.add_argument('--save-period', type=int, default=-1, help='Save checkpoint every x epochs (disabled if < 1)')
        self.parser.add_argument('--local_rank', type=int, default=-1, help='DDP parameter, do not modify')
        self.parser.add_argument('--nohalf', action='store_true', help='not use half to train')
        self.parser.add_argument('--quantize', action='store_true', help='run QAT with yolo')
        self.parser.add_argument('--BackendType', type=str, default='Tensorrt', help='backend for QAT deployment')
        self.parser.add_argument('--entity', default=None, help='W&B: Entity')
        self.parser.add_argument('--upload_dataset', action='store_true', help='W&B: Upload dataset as artifact table')
        self.parser.add_argument('--bbox_interval', type=int, default=-1, help='W&B: Set bounding-box image logging interval')
        self.parser.add_argument('--artifact_alias', type=str, default='latest', help='W&B: Version of dataset artifact to use')
        self.parser.add_argument('--sparse', action='store_true', help='train with channel sparsity regularization')
        self.parser.add_argument('--sparse-rate', type=float, default=0.001, help='scale sparse rate')

    def parse(self, known=False):
        return self.parser.parse_known_args()[0] if known else self.parser.parse_args()

arg_parser = ArgumentParser()
opt = arg_parser.parse()
