...
class ArgumentParser:
    ...
    def _add_arguments(self):
        ...
        self.parser.add_argument('--iou-thres', type=float, default=0.5, help='NMS IoU threshold')
        self.parser.add_argument('--task', default='val', help='train, val, test, speed or study')
        self.parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
        self.parser.add_argument('--single-cls', action='store_true', help='treat as single-class dataset')
...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        if plots and batch_i < 3:
            ...
            f = f'{save_dir}/val_batch{batch_i}_labels.jpg'  # labels
            Thread(target=plot_images, args=(img, targets, paths, f, names), daemon=True).start()
            #f = save_dir / f'val_batch{batch_i}_pred.jpg'  # predictions
            f = f'{save_dir}/val_batch{batch_i}_pred.jpg'  # predictions
